...
int main(int argc, char** argv){
	...
	int maxTime  = (argc > 9) ? atoi(argv[9]) : 2*60;
	...
	for(int branch = 0; branch < totalBranches; branch++) {
		...
		for(int attr = 0; attr < p; attr++){
			...
			if(usedAttr >= 0.5) {
				cout << endl << "Branch " << branch << ",\tParam. " << attr << ",\tValue " << std::setprecision(6) << splitValue << endl;
			}
			else {
	...
	for(int leaf = firstLeaf; leaf < maxNos; leaf++) {
		...
		for(int k_class = 0; k_class < K; k_class++) {
			...
			if(labeledLeaf) {
				cout << endl << "Leaf " << leaf << "\tclasse: " << k_class << "\tN Elements: " << cplex.getValue(N[k_class][leaf]) << endl;
			}
			else {
